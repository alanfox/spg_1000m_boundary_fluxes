...
def interpolate_grids(data2, interpolation_scheme):
    ...
    interpolation_scheme: strings
            Valied values are: t2u, t2v, u2v, v2u, t2f
    ...
    """
    data = data2.copy()
    if interpolation_scheme not in ["t2u", "t2v", "u2v", "v2u", "t2f"]:
    ...
    if interpolation_scheme == "u2v":
        for v in data.data_vars:
            data[v] = ((xr.concat([data[v],
                                   data[v].shift(x=1),
                                   data[v].shift(y=-1),
                                   data[v].shift(x=1, y=-1)],dim='new'))
                          .mean(dim='new',skipna=True))
...
def shift_grids_F(gridT, mesh_hgr, mesh_zgr, mask,
                coords_to_keep=('time_counter', ),
                vars_to_keep=('votemper', 'vosaline','sossheig'),
                var_e1v='e1v', var_e2v='e2v', var_e1u='e1u', var_e2u='e2u',
                var_e1t='e1t', var_e2t='e2t', var_e3u='e3u', var_e3v='e3v',
                var_e3t='e3t'):
    """Interpolate all input grids onto U-points and V-points and delete
    ...
    Returns
    -------
    gridF : xarray.Dataset
        Dataset of shape (t,z,y,x) containing all variables
        interpolated to F-point
    ...
    gridF = interpolate_grids(gridT, "t2f")
# # remove spurious z field in 2d fields
#     gridF["sossheig"] = gridF.sossheig.isel(z=0)
    return gridF
...
def select_section(ji, gridU, gridV, mesh_hgr=None, mesh_zgr=None, mask=None,
    ...
    if (("dvbydy" in gridU.data_vars) & ("dubydx" in gridV.data_vars)):
        gridU = gridU.rename({'dvbydy': 'du_by_ds_along'})
        gridV = gridV.rename({'dubydx': 'du_by_ds_along'})
    if (("dvbydx" in gridU.data_vars) & ("dubydy" in gridV.data_vars)):
        gridU = gridU.rename({'dvbydx': 'du_by_ds_normal'})
        gridV = gridV.rename({'dubydy': 'du_by_ds_normal'})
    ...
    if (("du_by_ds_along" in gridU.data_vars) & ("du_by_ds_along" in gridV.data_vars)):
        gridU['du_by_ds_along'] = gridU['du_by_ds_along'] * jjdiff
        gridV['du_by_ds_along'] = gridV['du_by_ds_along'] * iidiff
    if (("du_by_ds_normal" in gridU.data_vars) & ("du_by_ds_normal" in gridV.data_vars)):
        gridU['du_by_ds_normal'] = gridU['du_by_ds_normal'] * jjdiff
        gridV['du_by_ds_normal'] = gridV['du_by_ds_normal'] * iidiff * (-1)
...
def select_section_F(ji, gridF, mesh_hgr=None, mesh_zgr=None, mask=None,
                   var_e1t='e1v', var_e2t='e2u'):
    """Form a section by selecting quantities at U or V, depending on the
        orientation of each segment.
        We define velocities and transports, that go to the right of the
        progression of the section, as positiv.
...
Parameters
    ...
    ji : Iterator of tuples.
        ...
        used to rotate velocites.
    gridF : xarray.Dataset
        Dataset of shape (t,z,y,x) holding variables at F-point.
        Use dataset returned by broken_line.reduce_dataset()
    mesh_hgr : xarray.Dataset, optional
        Dataset of shape (1,y,x) with horizontal grid scale factors.
        Variable names can be given below.
    ...
    jjdiff = jj.diff('c')
    # We have one point more than segments
    gridF = gridF.isel(x=ii[:], y=jj[:])
    section = gridF
    section = section.assign({'ii': ii[:]})
    section = section.assign({'jj': jj[:]})
    
    if mesh_hgr:
        # Select lat
        gphif = mesh_hgr.gphif.isel(x=ii[:], y=jj[:])
        section = section.assign({'lat': gphif})
        # Select lon
        glamf = mesh_hgr.glamf.isel(x=ii[:], y=jj[:])
        section = section.assign({'lon': glamf})
    if mesh_zgr:
        section['z'] = mesh_zgr['nav_lev'].values
    return section
